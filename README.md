...
**いま販売は停止中**（2026-09-20 〜）。`index.html` の購入ボタンは外してあり、
価格カードには「現在、販売を停止しています / Sales are currently paused」が出る。
**無料トライアル（ダウンロード）は止めていない。**

### 🔴 止めるところは 2 つある。片方だけでは止まらない

| 場所 | 何が止まるか |
| --- | --- |
| `index.html` の購入ボタン | LP から決済へ**行く導線** |
| SendOwl の商品設定 | **決済ページそのもの**（`https://sowl.co/p/wr9dp`） |

**LP のボタンを外しても、決済ページは生きたまま。**
URL はメール・ブックマーク・過去のスクリーンショットに残っているので、
**SendOwl 側で商品を非公開（販売停止）にするまで、知っている人は買える。**

SendOwl の管理画面 → Products → 該当商品 → 公開を止める（下書きに戻す／無効化）。
**止めたあと、シークレットウィンドウで `https://sowl.co/p/wr9dp` を開いて
買えないことを確かめる。** ログイン中のブラウザでは見え方が変わる。

### 再開するとき

1. **SendOwl を先に開ける。** LP を先に戻すと、その数分のあいだに
   買おうとした人が死んだ決済ページへ落ちる
2. `index.html` の「販売を停止しています」の `<div>` を消して、
   `data-checkout` 付きの購入ボタンを戻す
   （消し方・戻し方はその場所の HTML コメントに書いてある）
3. 同意ダイアログ（`#consent`）とその JS は**残してある。**
   リンクを戻すだけで「同意 → 決済」の流れが再び効く
4. 戻したあと、**実際にボタンを押して決済ページまで行けることを確かめる**
