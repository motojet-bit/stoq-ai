...
いまは `lp/hero.svg`（ローソク足チャート）を敷いている。
...
暗いオーバーレイ（`.hero::after`、`rgba(13, 17, 23, 0.85)`）が上に乗るので、
**明るい画像に替えても文字は読める**。

### hero.svg について

- **ローソク足だけで描く。** 移動平均線やトレンドラインは入れない
  （線を引くと「そのラインで売買せよ」という含みが出て、投資助言に寄って見える）
- 値動きは「緩やかな上昇 → 放物線的な暴騰 → 鋭い急落」。
  LP の主張（熱狂に飛び乗らない）と同じ絵を背景に置いている
- ぼかしはかけていない。ローソク足だと分かる粒度で見せたいので、
  読みやすさはオーバーレイ側で確保する
...
- **index.html からはリンクしない。** gh-pages に置く以上 URL は誰でも踏めるので、
  検討中の価格が実売価格と取り違えられないよう `noindex, nofollow` を入れてある
