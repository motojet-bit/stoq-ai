...
**Step 1 / 1.1 / 2 / 3 / 4 / 4.1 / 5 / 5.1 / 5.2 / 5.3 ✅ 完了**
**Phase 2 の主要機能がひととおり動作し、自動テスト 92 件で保護されている**

### 自動テスト

```
npm run verify   # typecheck → test → build を通しで実行
npm run test     # フロントエンド 57 件（vitest）
cargo test --manifest-path src-tauri\Cargo.toml --lib   # Rust 35 件
```

| 対象 | 件数 | 主な検証内容 |
| --- | --- | --- |
| フロントエンド | 57 | リサイズ境界値 / フォント範囲 10〜20px / 配置の正規化 / 分析結果パーサのエッジケース / プロンプト予算 / トークン概算 |
